...
def main():
    ...
    logging.debug('{} voxels in brainmask'.format(idx_mask.shape))

    # get the seed time series
    logging.debug('unique rois are {}'.format(np.unique(rois_data)))
    idx = np.where(rois_data == int(roi))[0]
    logging.debug('{} voxels have roi number {}'.format(idx.shape, roi))
